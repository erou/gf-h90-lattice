...
\author{
  ...
  \alignauthor Hugues Randriam\footnote{thanks ANR-15-CE39-0013 project Manta}\\ %%% TODO: mettre Manta au bon endroit
  \affaddr{LTCI, T\'el\'ecom ParisTech}\\
  \email{randriam@enst.fr}
  \alignauthor \'Edouard Rousseau\footnote{thanks DIM Math'Innov}\\ %%% TODO: idem
...
running on Julia 1.1.0, and Nemo's current version of Flint\footnote{https://github.com/wbhart/flint2}. Critical routines are written in
...
different possible choices of characteristics, so we choose $p=5$.

Since Algorithm~\ref{algo:std_embed} require decorated finite
...
To measure the execution of Algorithm~\ref{algo:decoration}, we let $\ell$ grow
from $1$ to $200$, and for each $\ell$ not divisible by $p$ such that the $\nu(\ell)$-th Conway
polynomial is available, we add the time needed to compute the Kummer algebra
$A_\ell$ and to decore it. We obtain a set of decorated Kummer algebras and for
each pair $(A_\ell, \alpha_\ell), (A_m, \alpha_m)$ of such algebras with
$\ell\,|\,m$ we measure
the time needed to embed the corresponding fields $\mathbb{F}_{p^\ell}$ into
$\mathbb{F}_{p^m}$.

\begin{figure}
  \centering
  \includegraphics[scale=0.2]{solve_h90-5.png}
  \caption{Decoration timings for $p=5$.}
  \label{fig:timings-decoration}
\end{figure}

\paragraph{Acknowledgements} This work was partly financed by ANR-15-CE39-0013
project Manta and by grants from R\'egion Ile-de-France.
