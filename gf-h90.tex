...
By Proposition~\ref{Phialpha} this happens precisely when $\a_m=\embedcyc{\ell}{m}(\a_\ell)$,
so necessarily $\a_m$ lies in the subfield $\F_{p}((\zeta_m)^{\frac{m}{\ell}})$ of $\F_{p}(\zeta_m)$.
...
Thus, the complete Kummer algebra of level $a$ is $A_{p^a-1}$.
\end{definition}
\begin{lemma}
...
\end{definition}
Thus Lemma~\ref{Kummer_bizarre} asserts all nonzero $\alpha_{p^a-1}$ are standard.
\begin{proposition}
\label{embedincomplete}
Let $\ell\,|\,m$ not divisible by $p$ be such that $A_\ell$ and $A_m$ have the same level $a=\lambda(\ell)=\lambda(m)$.
Assume $A_\ell$ and $A_m$ come equipped with standard $\alpha_\ell$ and $\alpha_m$, respectively.
Then there is a unique Kummer embedding $\embedalg{l}{m}:A_\ell\hookrightarrow A_m$
...
Let $l$ be an integer not divisible by $p$.
Then $A_\ell$ admits a standard $\alpha_\ell$,
unique up to a $\ell$-th root of unity.
\end{proposition}
\begin{proof}
Let $\alpha'_\ell$ be any nonzero solution~\eqref{h90} for $\zeta_\ell$.
...
Then $\Phi(\alpha'_\ell)$ and $(\alpha_{p^a-1})^{\frac{p^a-1}{\ell}}$ are two nonzero solutions~\eqref{h90}
...
\begin{algorithm_endline}
  ...
  {$\alpha_\ell\in A_\ell$ standard, $P$ a standard irreducible polynomial of degree $\ell$,
  and $s\in\F_{p^\ell}$ a generating element inducing $\F_{p^\ell}\simeq\F_{p}[T]/(P)$.}
  {algo:standard}
\item Compute the Kummer algebras $A_\ell$ and $A_m$.
\item Find $\alpha_\ell\in A_\ell$ a standard solution of \eqref{h90} for $\zeta_\ell$.
\item Compute $P$ the minimal polynomial of $\first{\alpha_{\ell}}{\zeta_\ell}$ over $\F_{p}$.
\item Return $\alpha_{\ell}$, $P$, and $\first{\alpha_{\ell}}{\zeta_\ell}$.
\vspace{.5\baselineskip}
\end{algorithm_endline}
...
\end{proposition}
\begin{proof}
We still assume that the fields of scalars of our algebras are compatibly
