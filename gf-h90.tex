...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \REQUIRE {$\F_{p^\ell}, \F_{p^m}$, for $\ell\,|\,m$ integers prime to $p$, a root system
      $\system=\{\zeta_\ell,\zeta_m\}$.}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \REQUIRE {$\F_{p^\ell}$, for $\ell$ an integer prime to $p$, a root system $\system$.}
...
standard polynomials induced by the root system of Conway polynomials
for $p=2$.
...
e.g., using the equations given in~\cite{Allombert02,brieulle2018computing}.
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    \REQUIRE {Decorated finite fields $(\F_{p^\ell},\alpha_\ell)$, $(\F_{p^m},\alpha_m)$, for $\ell\,|\,m$ integers prime to $p$, a root system $\system$.}
    \ENSURE {$s\in\F_{p^\ell}, t\in\F_{p^m}$, such that the assignation $s\mapsto t$
...
\begin{proof}
  ...
  Now, to computed a Kummer embedding of $(\F_{p^\ell},\alpha_\ell)$
  ...
  We start by computing the minimal polynomial of $\zeta_m$ over
  $\F_p(\zeta_\ell)$ in $O(\M(m^2))$ operations using the algorithm
  of~\cite{shoup99}, from which we deduce the matrix of the trace form
  from $\F_p(\zeta_m)$ to $\F_p(\zeta_l)$ in $O(\M(m\ell))$ operations
  (see~\cite[Th.~3.1]{rouiller99}). %
  Now, if $\eta$ is any element such that
  $\Tr_{\F_q(\zeta_m)/\F_q(\zeta_\ell)}=1$, the map
  $\pi:x\mapsto\Tr_{\F_q(\zeta_m)/\F_q(\zeta_\ell)}(\eta x)$ is
  $\F_p(\zeta_\ell)$-linear, and agrees with the embedding defined by
  $(\zeta_m)^{\frac{m}{\ell}}\mapsto\zeta_\ell$ on
  $\F_p\bigl((\zeta_m)^{\frac{m}{\ell}}\bigr)$. %
  Hence, the composition of $\pi$ with the projection on the first
  coordinate is $\F_p$-linear, and is obtained by multiplying the
  first row of the trace form with the multiplication-by-$\eta$
  matrix, in $O(m\M(m))$ operations. %
  Once we have obtained the vector of this linear form, we can
  evaluate it on the scalar part of $(\alpha_m)^{\frac{m}{\ell}}$ in
  $O(m^2)$ operations, to obtain the final result.
\end{proof}
