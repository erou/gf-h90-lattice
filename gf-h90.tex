...
Algorithm~\ref{algo:decoration}. The bottleneck of the decoration being the root extraction.
...
$A_\ell$ and to decore it. We
then embed the prime field $\mathbb{F}_{p}$ in all the finite fields
$\mathbb{F}_{p^\ell}$ that we have decorated, that leads to 
Figure~\ref{fig:timings}

\begin{figure}[h]
 \includegraphics[scale=.24]{solve_h90-3.png}
 \includegraphics[scale=.24]{embed-3.png}
  \caption{Timings of the decoration (left) and of the embeddings from
    $\mathbb{F}_{p}$ (right) for $p=3$.}
  \label{fig:timings}
\end{figure}
...
$p=2$ the
first ten standard polynomials of degree not divisible by $2$ are
