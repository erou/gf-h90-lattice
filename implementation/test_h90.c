...
int main() {
  ...
  for (int i = 1 ; i < 40 ; i++) {
    ...
    nmod_poly_init(fact, tensor_p(A));
    ...
    fq_nmod_ctx_init_modulus(A.R, fact, "Z");

    fq_nmod_poly_init(h90, A.L);
    solve_h90(h90, state, A);
    
    if (!is_h90(h90, A)) {
      flint_printf("Not an H90 solution of order %d: ", n);
      fq_nmod_poly_print(h90, A.L);
      flint_printf("\n");
    } else {
      flint_printf(".%d.", n);
    }
